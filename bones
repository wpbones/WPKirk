...
namespace Bones\SemVer\Traits {
  ...
  trait Incrementable
  {
     ...
     *
     * @return \Bones\SemVer\Version|\Bones\SemVer\Traits\Incrementable This Version object
     */
...
namespace Bones {
  ...
  class BonesCommandLine
  {
     ...
     * @return mixed|array
     */
    protected function arguments(int $index = null): mixed
    {
      $argv = $_SERVER['argv'];
  ...
  o   o   o O--o      O--o  o-o o-o  o-o o-o
   \ / \ /  |         |   | | | |  | |-\'  \
     ...
     *
     */
    protected function rename($args): void
    {
      if ($this->isHelp()) {
        $this->info('Usage:');
      ...
      $files = array_filter(array_map(function ($e) {
        ...
        if (
          str_contains($e, "node_modules") ||
          str_contains($e, "vendor/wpbones/wpbones/src/Console/bin/bones")
        ) {
          return false;
        }
     ...
     *
     * @param null $index Optional. Index of param.
     *                    If NULL will be returned the whole array.
     *
     * @return array|null
     */
    protected function commandParams($index = null): ?array
    {
    ...
    protected function handle(): void
    {
      ...
      elseif ($this->isCommand('require')) {
        $this->requirePackage((string) $this->commandParams(0));
      } // Version
      ...
      elseif ($this->isCommand('make:controller')) {
        $this->createController((string) $this->commandParams(0));
      } // make:console {command_name}
      elseif ($this->isCommand('make:console')) {
        $this->createCommand((string) $this->commandParams(0));
      } // make:cpt {className}
      elseif ($this->isCommand('make:cpt')) {
        $this->createCustomPostType((string) $this->commandParams(0));
      } // make:shortcode {className}
      elseif ($this->isCommand('make:shortcode')) {
        $this->createShortcode((string) $this->commandParams(0));
      } // make:provider {className}
      elseif ($this->isCommand('make:provider')) {
        $this->createProvider((string) $this->commandParams(0));
      } // make:ajax {className}
      elseif ($this->isCommand('make:ajax')) {
        $this->createAjax((string) $this->commandParams(0));
      } // make:ctt {className}
      elseif ($this->isCommand('make:ctt')) {
        $this->createCustomTaxonomyType((string) $this->commandParams(0));
      } // make:widget {className}
      elseif ($this->isCommand('make:widget')) {
        $this->createWidget((string) $this->commandParams(0));
      } // make:model {className}
      elseif ($this->isCommand('make:model')) {
        $this->createModel((string) $this->commandParams(0));
      } // make:eloquent-model {className}
      elseif ($this->isCommand('make:eloquent-model')) {
        $this->createEloquentModel((string) $this->commandParams(0));
      } // make:api {className}
      elseif ($this->isCommand('make:api')) {
        $this->createAPIController((string) $this->commandParams(0));
      } // else...
      else {
        $extended = false;
     ...
     */
    protected function deploy(string $path): void
    {
     ...
     */
    protected function createMigrate($tablename): void
    {
     ...
     */
    protected function createController(string $className): void
    {
     ...
     * @param string $className The class name
     */
    protected function createCommand(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createCustomPostType(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createShortcode(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createProvider(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createAjax(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createCustomTaxonomyType(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createWidget(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createEloquentModel(string $className): void
    {
      if ($this->isHelp($className)) {
     ...
     * @param string $className The class name
     */
    protected function createAPIController(string $className): void
    {
      if ($this->isHelp($className)) {
